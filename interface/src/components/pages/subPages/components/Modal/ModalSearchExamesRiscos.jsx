...
const ModalSearchExamesRiscos = ({ onCancel, isOpen, children, childId, atualizaDados }) => {
  const [searchTerm, setSearchTerm] = useState('');
  const [examesNaoVinculados, setExamesNaoVinculados] = useState([]);
  const { getRiscosExamesSemVinculo } = useAuth(null);
  ...
  const handleNaoVinculados = async () => {
    try {
      ...
      console.error('Erro ao buscar exames não vinculados:', error);
    }
  }
  ...
  const handleItemClick = async (item) => {
    try {
      ...
      const response = await fetch(`${connect}/risco_exame?${queryParams}`, {
        method: 'POST',
        headers: {
      ...
      if (response.ok) {
        ...
        handleNaoVinculados();
        // Lógica após o sucesso da requisição
      } else {
        console.error('Falha ao vincular exame ao risco.');
  ...
  return (
    <div className="fixed inset-0 flex items-center justify-center z-50">
      ...
      <div className="modal-container w-2/6 bg-white mx-auto rounded-xl z-50 overflow-y-auto px-8 py-4 max-h-[80vh]">
        ...
        {examesNaoVinculados.examesNaoVinculados.length > 0? (
  examesNaoVinculados.examesNaoVinculados.map((item, i) => (
    <li
      key={i}
...
export default ModalSearchExamesRiscos;
