...
function getRiscosFromPermissions(tenantCode) {
        ...
        pool.getConnection((err, conn) => {
            ...
            conn.query(query, [tenantCode], (err, results) => {
                ...
                if (Array.isArray(results) && results.length > 0) {
                    ...
                    const globals = {
                        queryGlobal: 'SELECT * FROM riscos WHERE (tenant_code =? OR tenant_code IS NULL)',
                        queryTenant: 'SELECT * FROM riscos WHERE (tenant_code =?)',
                    };
...
export default getRiscosFromPermissions;
